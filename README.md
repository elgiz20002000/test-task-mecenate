...
- API комментариев не поддерживает сортировку (`sort`/`order` параметры отсутствуют).
- Из-за этого приложение не может гарантировать отображение последних комментариев в начале списка по запросу к бэкенду.